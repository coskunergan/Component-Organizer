...
void CO::useDefaultData()
{
    ...
    top = new Label(tr("Rayban"));
    top->addLeaf(new Label(tr("Kemik"), top));
    top->addLeaf(new Label(tr("Yuvarlak"), top));
    m_topLabels.append(top);

    top = new Label(tr("Prada"));
    m_topLabels.append(top);

    top = new Label(tr("Chanel"));
    m_topLabels.append(top);

    top = new Label(tr("Dolce & Gabbana"));
    m_topLabels.append(top);

    top = new Label(tr("Osse"));
    m_topLabels.append(top);
}
...
bool CO::writeXML(const QString &filePath)
{
    ...
    foreach(Component *c, m_components)
    {
        ...
        {
            stream.writeStartElement("stok");
            stream.writeAttribute("gurup", s->package()->name());
            stream.writeAttribute("value", QString::number(s->stock()));
            stream.writeAttribute("min", QString::number(s->lowValue()));
